...
def setup():
    ...
    if platform.system() == 'Darwin':
        sh('python -m pip install -U tkmacosx')
    elif platform.system() == 'Windows':
        print("Windows detected, skipping tkmacosx installation.")

@task
def test():
    sh('python -m coverage run --source src --omit="src/mastermind_gui.py" -m unittest discover -s test')
...
@task    
def run():
    sh('python src/mastermind_gui.py')
